...
def run_discord_bot():
    ...
    @client.tree.command(name="chat", description="Request support from SYN")
    async def chat(interaction: discord.Interaction, *, message: str):
        if client.is_replying_all == "True":
            await interaction.response.defer(ephemeral=False)
            await interaction.followup.send(
                "> **WARN: You already on replyAll mode. If you want to use the Slash Command, switch to normal mode by using `/replyall` againWARNING: You are currently in replyAll mode. If you wish to utilize the Slash Command, switch back to normal mode by using /replyall once more.**")
            logger.warning("\x1b[31mYou are currently in replyAll mode, and the use of slash commands is not available.\x1b[0m")
            return
    ...
    async def private(interaction: discord.Interaction):
        ...
        if not client.isPrivate:
            ...
            await interaction.followup.send(
                "> **INFORMATION: The upcoming response will be sent through a private reply. If you wish to switch back to public mode, please utilize the `/public` command.**")
        else:
            logger.info("You are currently in private mode.")
            await interaction.followup.send(
                "> **WARNING: You are already in private mode. If you wish to switch to public mode, please use the /public command.**")
    ...
    async def public(interaction: discord.Interaction):
        ...
        if client.isPrivate:
            client.isPrivate = not client.isPrivate
            await interaction.followup.send(
                "> **INFORMATION: The forthcoming response will be sent directly to the channel. If you desire to switch back to private mode, please employ the /private command.**")
            logger.warning("\x1b[31mSwitch to public mode\x1b[0m")
        else:
            await interaction.followup.send(
                "> **WARNING: You are currently in public mode. If you wish to switch to private mode, please utilize the /private command.**")
    ...
    async def replyall(interaction: discord.Interaction):
        ...
        if client.is_replying_all == "True":
            ...
            await interaction.followup.send(
                "> **INFORMATION: The next response from the bot will be directed through the Slash Command. If you wish to switch back to replyAll mode, simply use /replyAll once again.**")
            logger.warning("\x1b[31mSwitch to normal mode\x1b[0m")
        elif client.is_replying_all == "False":
            client.is_replying_all = "True"
            await interaction.followup.send(
                "> **INFORMATION: The bot will now disable the Slash Command and respond exclusively to messages within this channel. If you wish to switch back to normal mode, please use /replyAll again.**")
    ...
    async def reset(interaction: discord.Interaction):
        ...
        logger.warning(
            f"\x1b[31m{client.chat_model} The bot has been reset successfully.\x1b[0m")
    ...
    async def help(interaction: discord.Interaction):
        ...
        - `/chat [message]` Request support from SYN
        - `/draw [prompt]` Generate an image SYN
        - `/switchpersona [persona]` Jailbreak SYN
                ...
                `based`: SEXY AI
                `oppo`: Say the opposite of what SYN would say
                `dev`: Developer Mode
...
""")
    ...
    @client.tree.command(name="draw", description="Generate an image SYN")
    ...
    async def draw(interaction: discord.Interaction, *, prompt: str, amount: int = 1):
        ...
        try:
            ...
            logger.info(
            f"\x1b[31m{username}\x1b[0m An inappropriate request was made.")

        except Exception as e:
            await interaction.followup.send(
                "> **ERROR: An error occurred.**")
            logger.exception(f"There was an error during image generation.{e}")


    @client.tree.command(name="switchpersona", description="jailbreak SYN")
    @app_commands.choices(persona=[
        ...
        app_commands.Choice(name="Developer Mode", value="dev"),
